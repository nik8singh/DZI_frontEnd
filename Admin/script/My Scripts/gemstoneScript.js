    ...
    $.ajax({
        ...
        success: function (result) {
            ...
            $.each(result, function (index, element) {
                ...
                if (i > 0) {
                    console.log("gemstoneID: " + element.gemstoneId + " || gemstoneName: " + element.gemstoneName + "|| gemstoneDescription: " + element.gemstoneDescription);
                    let r = "<tr class=\"tbl-item\" id=\"" + element.gemstoneId + "\"> <td class=\"td-block\" style=\"width: 100%\">" + "<p class=\"date\">" + createdDate + "</p>" + "<textarea  class=\"title itemEditableFields itemTitle \"  disabled>" + element.gemstoneName + "</textarea>" + "<textarea class=\"desc itemDesc itemEditableFields\" disabled>" + element.gemstoneDescription + "</textarea>" + "<p class=\"like\" style=\"float: left\"> Last Updated: " + updatedDate + "</p>" + "<a href=\"#\" class=\"itemButtons editItem\" >Edit</a>" + "<a class=\"updateItem itemButtons\" href=\"#\" hidden>Update</a>" + "<a class=\"cancelUpdateItem itemButtons\" href=\"#\" hidden>Cancel</a>" + "<a class=\"deleteItem itemButtons\" href=\"#\" hidden>Delete Gemstone</a>" + "</td></tr>";
                    $(".demo-tbl").prepend(r);
                } else {
                    console.log("First gemstoneID: " + element.gemstoneId + " || gemstoneName: " + element.gemstoneName + "|| gemstoneDescription: " + element.gemstoneDescription);
                    $('.itemCreatedDate').text(createdDate);
                    $('.itemUpdatedDate').text("Last Updated: " + updatedDate);
                    $('.itemTitle').text(element.gemstoneName);
                    $('.itemDesc').text(element.gemstoneDescription);
                    $('.tbl-item').attr('id', element.gemstoneId);
                    $('.editItem').show();
                }
                i++;
            });
...
function createJsonObject(newItem) {
    let myObject = {
        gemstoneName: gemName.val(),
        gemstoneDescription: gemDesc.val()
    };

    if (newItem === 0)
        myObject.gemstoneId = itemId;
...
$(tablesDivision).on('click', '.updateItem', function () {
    ...
    $.ajax({
        ...
        error: function (request, status, error) {
            showAlert("alert-danger", "Error gemstone Not updated. Please try again later", self);
            console.log("Status: " + status);
            console.log("error: " + error);
    ...
    if (r === true) {
        ...
        $.ajax({
            ...
            success: function (result, status) {
                ...
                showAlert("alert-warning", "<strong>" + name + "</strong> Gemstone Deleted", ".page-header");
            },
            error: function (request, status, error) {
                showAlert("alert-danger", "Error: gemstone Not deleted", ".addNewItem");
                console.log("Status: " + status);
                console.log("error: " + error);
                console.log("response: " + request.responseText);
            },
...
$(tablesDivision).on('click', ".addNewItem", function () {
    ...
    $.ajax({
        ...
        error: function (request, status, error) {
            showAlert("alert-danger", "Error: gemstone Not added", ".addNewItem");
            console.log("Status: " + status);
            console.log("error: " + error);
            console.log("response: " + request.responseText);
        },
